...
bool                PManip::sKeyPressed = FALSE;
//SoCoordinate3*    PManip::sNewCoords;
	...
	case kDeletePoint:
		{
			if (sSelectedNode->isOfType(SoPointSet::getClassTypeId()))
			{
				...
				TRACE("kDeletePoint [ %d ]\n", sSelectedCoord);
				sSelectedEntity->DeletePoint(sSelectedCoord);
			}
			else
				sSelectedCoord = -1;
		}
	...
	case kMovePoint:
		{
			...
			if (sSelectedNode->isOfType(SoPointSet::getClassTypeId())) 
			{
				...
				//else if (sMouseButton == 1 && (sKeyPressed == TRUE))
            else if (sMouseButton == 1 )
				{
               //move point parallel to view plane... !!!
					TRACE("shiftDown\n");
					SbVec3f tCoord = sSelectedEntity->fCoords->point[sSelectedCoord];
					//SbVec3f tCoord2 = SbVec3f (tCoord[0], tCoord[1], tCoord[2] + 10);
					//sProjectionPlane = SbLine(tCoord, tCoord2);
					//sPlaneProjector->setOrientToEye(TRUE);
               ...
               SbVec3f downVector(0,0,-1);
               SbVec3f newVector;
               
               rotMatrix.multVecMatrix(downVector, newVector);
               // construt a plane with camera orientation as plane normal, and 
               // through the selected point...
               SbPlane viewPlane(newVector, tCoord);
               sPlaneProjector->setPlane(viewPlane);
...
void
PManip::KeyPressed(void *data, SoEventCallback *eventCB)
{
	TRACE("key pressed\n");
	const SoEvent *ev = eventCB->getEvent();
	if(SO_KEY_PRESS_EVENT(ev,ANY) )     
	
	//if (ev->isKeyPressEvent(ev,SoKeyboardEvent::LEFT_SHIFT))
	{
		sKeyPressed = TRUE;
	}
...
{
   if (sSelectedEntity == NULL) return;
   ...
   const SoEvent *ev = eventCB->getEvent();
	...
	case kMovePoint:
		...
		// !!! added this section for no middle button...
		if (sSelectedEntity != NULL && sMouseButton == 1 && (sKeyPressed == TRUE))
      {  /// actually this is for testing projecting to the plane parallel to screen that passes through selected point
			TRACE("key down\n");
			SbVec3f newPosition;
		...
		else if (sSelectedEntity != NULL && sMouseButton == 1)
		{
			SbVec3f newPosition;
			theManip->ProjectMouseToPlane(ev, newPosition);
			sSelectedEntity->fCoords->point.set1Value(sSelectedCoord, newPosition);
